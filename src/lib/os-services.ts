...
export const forecastService = {
  getLatest: (
    commodityId: number,
    callback: (forecasts: Forecast[]) => void,
  ) => {
    const q = query(
    ...
    return onSnapshot(
      q,
      (snapshot) => {
        const forecasts = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as Forecast,
        );
        callback(forecasts);
      },
      (error) => {
        handleFirestoreError(
          error,
          OperationType.LIST,
          `forecasts?commodityId=${commodityId}`,
        );
      },
    );
  },
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        const records = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as ProductionRecord,
        );
        callback(records);
      },
      (error) => {
        handleFirestoreError(
          error,
          OperationType.LIST,
          `production_records?farmerId=${farmerId}`,
        );
      },
    );
  },
