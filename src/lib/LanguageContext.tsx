...
const translations: Record<Language, Record<string, string>> = {
  en: {
    ...
    heroSubtitle:
      'One platform for real‑time market prices, AI forecasts, direct farm‑to-buyer sales, and a permanent digital portfolio for every farmer.',
    signIn: 'Sign In',
  ...
  ta: {
    ...
    heroSubtitle:
      'இந்தியாவிற்கான நேரடி கொள்முதல், உகந்த தளவாடங்கள் மற்றும் வெளிப்படையான சந்தை நெறிமுறைகள்.',
    signIn: 'உள்நுழைய',
  ...
  kn: {
    ...
    heroSubtitle:
      'ನೇರ ಸಂಗ್ರಹಣೆ, ಆಪ್ಟಿಮೈಸ್ಡ್ ಲಾಜಿಸ್ಟಿಕ್ಸ್ ಮತ್ತು ಭಾರತಕ್ಕಾಗಿ ಪಾರದರ್ಶಕ ಮಾರುಕಟ್ಟೆ ಪ್ರೋಟೋಕಾಲ್ಗಳು.',
    signIn: 'ಸೈನ್ ಇನ್',
  ...
  te: {
    ...
    heroSubtitle:
      'నేరుగా సేకరణ, ఆప్టిమైజ్ చేసిన లాజిస్టిక్స్ మరియు భారతదేశం కోసం పారదర్శక మార్కెట్ ప్రోటోకాల్‌లు.',
    signIn: 'సైన్ ఇన్',
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        else if (latitude >= 11 && latitude <= 19 && longitude >= 74 && longitude <= 78.5)
          detected = 'kn';
        else if (latitude >= 15.5 && latitude <= 22 && longitude >= 72.5 && longitude <= 81)
          detected = 'mr';
        else if (latitude >= 12 && latitude <= 19.5 && longitude >= 77 && longitude <= 85)
          detected = 'te';
        else if (latitude >= 21.5 && latitude <= 27.5 && longitude >= 85.5 && longitude <= 90)
          detected = 'bn';
        else if (latitude >= 20 && latitude <= 37 && longitude >= 68 && longitude <= 97)
          detected = 'hi';
