...
const WeatherIcon = ({ condition }: { condition: string }) => {
  ...
  if (normalizedCondition.includes('clear')) return <Sun size={20} className="text-yellow-500" />;
  if (normalizedCondition.includes('rain')) return <CloudRain size={20} className="text-blue-500" />;
  if (normalizedCondition.includes('storm')) return <CloudLightning size={20} className="text-primary" />;
