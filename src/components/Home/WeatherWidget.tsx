...
export const WeatherWidget = () => {
  ...
  const Icon = condition.includes("clear")
    ? Sun
    : condition.includes("rain")
      ...
      : condition.includes("storm")
        ? CloudLightning
        : Cloud;
  const iconColor = condition.includes("clear")
    ? "text-yellow-500"
    : condition.includes("rain")
      ? "text-blue-500"
      : condition.includes("storm")
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        <span className="mono text-[10px] text-primary font-medium normal-case ">
          Atmosphere Node
        </span>
        <div className="flex items-center gap-2">
          <Icon size={20} className={iconColor} />
          <span className="display text-3xl font-light text-white">
            {weather.temp}°C
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-0.5">
        <span className="mono text-[9px] text-white/30 normal-case ">
          {weather.location}
        </span>
        <span className="display text-sm font-medium text-white normal-case tracking-tight italic">
          {weather.condition}
        </span>
      </div>
