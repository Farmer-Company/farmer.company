...
export const ConfigurePage = () => {
    ...
    return (
      <div className="pt-40 flex flex-col items-center gap-8 px-6 text-center">
        <div className="space-y-4 max-w-2xl">
          <h2 className="text-2xl font-medium normal-case tracking-tight text-white">
            OS Parameter Configuration
          </h2>
          <p className="text-foreground-muted leading-relaxed">
            Personalise your Digital Orchard operating environment — commodity
            watchlists, regional alerts, price threshold notifications, forecast
            subscriptions, and logistics route preferences.
          </p>
          <p className="mono text-[10px] normal-case text-white/40 pt-4">
            Sign in or create your beta account to access configuration.
          </p>
        </div>
        <Button onClick={() => navigate("/signin")} className="h-14 px-12 mt-4">
          Initialize Sign-In
        </Button>
      </div>
    );
  ...
  return (
    <div className="pt-32 px-10 min-h-screen bg-background text-white pb-32">
      ...
      <p className="text-foreground-muted normal-case text-sm">
        Configure your Digital Orchard Identity
      </p>
      ...
      <div className="mt-12 max-w-2xl bg-[#0D0D0D] border border-white/5 p-12 overflow-hidden relative">
        <AnimatePresence>
          {isSubmitting && (
            ...
            >
              ...
              <span className="mono text-[10px] normal-case text-primary">
                Synchronizing Portals...
              </span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="flex justify-between mb-12">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className={`h-1 flex-1 mx-1 rounded-full transition-all duration-500 ${step >= i ? "bg-primary" : "bg-white/10"}`}
            />
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div
              ...
              className="space-y-8"
            >
              <h2 className="text-2xl font-medium normal-case tracking-tight">
                Basic Specification
              </h2>
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-medium text-foreground-muted normal-case ">
                    Full Name
                  </label>
                  <input
                    type="text"
              ...
              <Button
                onClick={nextStep}
                disabled={!fullName}
                className="w-full h-14 text-xs font-medium normal-case "
              >
                Next Phase →
              </Button>
            </motion.div>
          ...
          {step === 2 && (
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-4">
                {[
                  ...
                  >
                    ...
                    <span className="mono text-sm font-medium normal-case group-hover:text-primary transition-colors">
                      {item.label}
                    </span>
                  </button>
                ))}
              ...
              <Button
                onClick={prevStep}
                variant="ghost"
                className="w-full h-14 text-[10px] normal-case font-medium text-white/40"
              >
                ← Back
              </Button>
            </motion.div>
          ...
          {step === 3 && (
            ...
            >
              <h2 className="text-2xl font-medium normal-case tracking-tight">
                {role?.toUpperCase()} Implementation
              </h2>
              ...
              <div className="space-y-6">
                ...
                {(role === "government" || role === "logistics") && (
                  <p className="text-sm text-foreground-muted italic">
                    Identifying systemic architecture parameters for public data
                    analysis...
                  </p>
                )}
              </div>
          ...
          {step === 4 && (
            ...
            >
              <h2 className="text-2xl font-medium normal-case tracking-tight">
                B2B Export Readiness
              </h2>
              <p className="text-sm text-foreground-muted font-light leading-relaxed">
                Enable global supply features and institutional offtake
                agreements by providing your export details.
              </p>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input
                    className="w-full bg-background border border-white/10 h-14 px-4 outline-none focus:border-primary"
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {["GlobalG.A.P.", "Organic", "Fair Trade", "ISO 22000"].map(
                      (cert) => (
                        <div
                          key={cert}
                          className="flex items-center gap-3 p-4 border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition-colors cursor-pointer"
                        >
                          <div className="w-4 h-4 border border-primary/40 rounded-sm" />
                          <span className="mono text-[10px] normal-case font-medium text-white/60">
                            {cert}
                          </span>
                        </div>
                      ),
                ...
                <div className="space-y-2">
                  ...
                  <select
                    className="w-full bg-background border border-white/10 h-14 px-4 outline-none appearance-none focus:border-primary"
